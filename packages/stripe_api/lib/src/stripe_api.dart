...
class StripeApi {
  ...
  /// Lists every invoice generated by the subscription with the given
  /// [subscriptionId], in any status.
  ///
  /// See https://docs.stripe.com/api/invoices/list.
  Future<List<StripeInvoice>> listInvoices({required String subscriptionId}) =>
      _fetchAllPages(
        path: 'invoices',
        queryParameters: {'subscription': subscriptionId},
        fromJson: StripeInvoice.fromJson,
        getId: (e) => e.id,
      );

  /// Voids the invoice with the given [invoiceId] and returns it as Stripe
  /// reports it afterwards.
  ///
  /// Stripe accepts this only on an `open` or `uncollectible` invoice, and
  /// the result is final: a voided invoice can no longer be paid.
  ///
  /// See https://docs.stripe.com/api/invoices/void.
  Future<StripeInvoice> voidInvoice({required String invoiceId}) async {
    final uri = _stripeUri(path: 'invoices/$invoiceId/void');
    final response = await _client.post(uri, headers: _authHeaders);

    if (response.statusCode != HttpStatus.ok) {
      throw StripeApiException.fromResponse(
        response,
        message: 'Failed to void invoice with id $invoiceId',
      );
    }

    return StripeInvoice.fromJson(
      jsonDecode(response.body) as Map<String, dynamic>,
    );
  }
