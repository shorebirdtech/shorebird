    ...
    return runScoped(() async {
      try {
        ...
        if (argReleaseVersion != null) {
          logger.detail(
            'Using release version $argReleaseVersion from argument.',
          );
          releaseVersion = argReleaseVersion;
        } else {
          logger
              .detail('No release version provided. Determining from bundle.');
...
Please re-run the release command for this version or create a new release.''');
        ...
        if (release.flutterRevision != currentFlutterRevision) {
          logger.info(
            'Release was built with Flutter revision '
  ...
  - build/app/intermediates/stripped_native_libs/{flavor}Release/out/lib''',
            ...
            for (final releaseArtifactPath in releaseArtifactPaths.entries) {
              ...
              try {
                ...
                patchArtifactBundles[releaseArtifactPath.key] =
                    PatchArtifactBundle(
                  arch: arch.arch,
                  path: diffPath,
                  hash: hash,
