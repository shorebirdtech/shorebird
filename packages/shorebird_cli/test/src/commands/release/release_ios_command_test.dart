    ...
    - shorebird.yaml''';
      ...
      group('when codesign is disabled', () {
        ...
        group('when build directory has non-default structure', () {
          test(
              'prints error and exits with code 70 if xcarchive does not exist',
              () async {
            setUpProjectRoot();
            Directory(
              p.join(
        ...
        test('creates unsigned release artifacts', () async {
          ...
          verify(
            () => codePushClientWrapper.createIosReleaseArtifacts(
              ...
              runnerPath:
                  any(named: 'runnerPath', that: endsWith('Runner.app')),
              isCodesigned: false,
            ),
          ).called(1);
        });
...
''');
    ...
    No profiles for 'com.example.co' were found'''),
      ...
      test('exits with code 70 if ipa build directory does not exist',
          () async {
        setUpProjectRoot();
